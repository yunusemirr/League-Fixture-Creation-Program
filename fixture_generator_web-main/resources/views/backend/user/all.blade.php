        ...
        <div class="shadow-sm card">
            ...
            <div class="card-body row gx-4 gy-4">
                ...
                <div class="col-12 col-md-6 col-lg-4 col-xl-2 form-group">
                    <label class="text-gray-400 form-label fs-7">Role Göre</label>
                    <select filter-by="role_id" class="form-control form-select form-select-sm" data-control="select2">
                        <option value="-1">Hepsi</option>
                        @foreach (["1" => "Öğrenci", "2" => "Veli", "3" => "Öğretmen", "4" => "Yönetici", "5" => "Admin"] as $role_id => $role)
                            <option @selected(request()->get('role_id', -1) == $role_id) value="{{ $role_id }}">{{ $role }}</option>
                        @endforeach
                    </select>
                </div>
...
<script>
    ...
    $(document).ready(function () {
        _dt = new HDataTable("table", {
            ...
            ajax: {
                url: "{{ route('user.index', ['datatable' => 'true']) }}",
                type: "GET",
                data: (d) => {
                    let filters = {};
            ...
            columns: [
                ...
                { title:'{{ __('models.group.name') }}', name:'name', data:'full_name'},
                {
